...
public class TestCRUD extends TestCase {
    ...
    private long saveObject(Object obj) throws Exception {
        ...
        try {
            ...
            } else if (obj instanceof MultiLineStringEntity) {
                id = ((MultiLineStringEntity) obj).getId();
    ...
    public void testEmptyLineStringEntity() throws Exception {
        LineStringEntity line = new LineStringEntity();

        LineString linestring = geomFactory.createLineString(new Coordinate[]{});
        line.setGeometry(linestring);
        line.setName("Empty geom Added by TestCRUD");
        long id = saveObject(line);
        LineStringEntity retrieved = (LineStringEntity) retrieveObject(
                LineStringEntity.class, id);
        assertTrue(retrieved.getGeometry().isEmpty());
    }

    public void testEmptyMultiLineStringEntity() throws Exception {
        MultiLineStringEntity entity = new MultiLineStringEntity();
        MLineString[] mlines = new MLineString[]{};

        MultiMLineString multiMLine = geomFactory.createMultiMLineString(mlines);
        entity.setGeometry(multiMLine);
        entity.setName("Added by TestCRUD");
        long id = saveObject(entity);
        MultiLineStringEntity retrievedEntity = (MultiLineStringEntity) retrieveObject(MultiLineStringEntity.class, id);
        MultiLineString retrievedGeom = (MultiLineString) retrievedEntity.getGeometry();
        assertTrue(retrievedGeom.isEmpty());
    }
}
